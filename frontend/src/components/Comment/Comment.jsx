...
export default function Comment({id,text,profilepic,name,createdAt, post_id}) {
    ...
    useEffect(() => {
      setLiked(data?.getCommentLikes.likes.filter(e => e.user_id == user.user_id).length > 0);
      setDisliked(data?.getCommentLikes.dislikes.filter(e => e.user_id == user.user_id).length > 0);
    }, [data])
...
<div className="commentBubble">
            ...
            {user.name == name ? <DeleteComment comment_id={id} post_id={post_id}/> : <p></p>}
</div>
